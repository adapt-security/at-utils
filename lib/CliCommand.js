...
export default class CliCommand {
  ...
  async run(destination, opts, command) {
    await this.setOptions(destination, opts, command);
    await this.runTask();
  }
  async runTask() {
    throw new Error('Should be overridden in subclass'); 
  }
  async setOptions(destination, opts, command) {
    ...
    this.options = {
      ...
      releaseData: await this.getReleaseData(),
      tag,
      ui
    };
  }
  ...
  async getReleaseData() {
    let currentVersion;
    try {
      currentVersion = await Utils.exec('git symbolic-ref -q --short HEAD || git describe --tags --exact-match', this.options.cwd);
    } catch(e) {}
    return {
      currentVersion,
