...
import { isAdaptModule } from './peerDeps.js'
...
export function extractVersions (lock) {
  ...
  for (const [key, entry] of Object.entries(packages)) {
    ...
    if (!isAdaptModule(name)) continue
...
export function diffVersions (oldVersions, newVersions) {
...
export function compareVersions (cwd) {
  ...
  try {
    ...
    if (pkg.name !== 'adapt-authoring') {
      throw new Error('This command must be run in the adapt-authoring root repository.')
    }
    currentVersion = pkg.version
  } catch (e) {
    if (e.message.startsWith('This command')) throw e
  ...
  const changes = diffVersions(oldVersions, newVersions)
