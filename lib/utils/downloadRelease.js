...
const GITHUB_ORG = 'adapt-security'
const DEFAULT_REPO = 'adapt-authoring'
...
export default async function downloadRelease ({ tag, targetDir, repo = DEFAULT_REPO, cleanInstall = true }) {
  ...
  const response = await fetch(`https://api.github.com/repos/${GITHUB_ORG}/${repo}/tarball/${tag}`, { headers: githubHeaders() })
  if (!response.ok) {
