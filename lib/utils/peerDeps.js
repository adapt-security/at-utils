...
import collectJsFiles from './collectJsFiles.js'

export const PREFIX = 'adapt-authoring-'

/**
 * Check whether a package name is an adapt-authoring module.
 * Handles both unscoped (adapt-authoring-*) and scoped (@scope/adapt-authoring-*) names.
 */
export function isAdaptModule (name) {
  if (name.startsWith(PREFIX)) return true
  if (name.startsWith('@')) {
    const unscoped = name.slice(name.indexOf('/') + 1)
    return unscoped.startsWith(PREFIX)
  }
  return false
}
 ...
 */
/**
 * Find adapt-authoring-* dependencies with outdated versions compared to the package index.
 * Returns an array of { dep, current, expected, section } objects.
 */
export function findOutdatedVersions (pkg, pkgIndex) {
  const outdated = []
  for (const section of ['dependencies', 'peerDependencies']) {
    for (const [dep, ver] of Object.entries(pkg[section] || {})) {
      if (!isAdaptModule(dep)) continue
      const info = pkgIndex.get(dep)
      if (!info) continue
      const expected = `^${info.version}`
      if (ver !== expected) {
        outdated.push({ dep, current: ver, expected, section })
      }
    }
  }
  return outdated
