...
export default class Utils {
  ...
  static async cloneRepo(options) {
    ...
    await this.exec(`npm ${options.cleanInstall !== false ? 'ci' : 'install'}`, options.cwd);
  }
  ...
  static async installLocalModules(options) {
    ...
    await Promise.all(options.modules.map(async m => {
      ...
      await this.cloneRepo({ repo: m, cwd: modulePath, cleanInstall: false });
