...
export default class Utils {
  ...
  static async installLocalModules(options) {
    const localRootFolder = 'local_adapt_modules';
    const workspaces = [];
    await Promise.all(options.modules.map(async m => {
      await this.cloneRepo({ repo: m, cwd: path.join(options.cwd, localRootFolder, m), cleanInstall: false });
      workspaces.push(`${localRootFolder}/${m}`);
    }));
    ...
    // need to run another npm install to pull in the local repos
    return this.exec('npm install', options.cwd);
  }
