...
class Utils {
  static async addModulePath(newPath) {
    if(!newPath.endsWith('/node_modules')) newPath += '/node_modules';
    try {
  ...
  static async saveConfig(rootDir, data) {
    ...
    const exportPrefix = Number(process.version.slice(1,3)) >= 16 ? 'export default' : 'module.exports =';
    await fs.writeFile(configPath, `${exportPrefix} ${JSON.stringify(config, null, 2)};`);
  }
