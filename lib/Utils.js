...
export default class Utils {
  ...
  static async registerSuperUserCmd(options = {}) {
    let email = options.superEmail;
    let password = options.pipePasswd ? await this.getPipedPassword() : '';
    let success = false;
    while(!success) {
      try {
        if(!email) {
          email = await await prompts([{
            type: 'text',
            name: 'email',
        ...
        await this.registerSuperUser(options.cwd, { email, password });
        success = true;
      } catch(e) {
  ...
  static async sendTestMail(email) {
    const config = (await import(pathToFileURL(path.join(process.cwd(), 'conf', `${process.env.NODE_ENV}.config.js`)))).default;
    const { host, port } = config['adapt-authoring-server'];
    const response = await fetch(`http://${host}:${port}/api/mailer/test`, { 
      method: 'POST', 
