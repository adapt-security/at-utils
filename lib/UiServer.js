...
export default class UiServer extends EventEmitter {
  constructor (options) {
    ...
    this._server.on('connection', socket => {
      ...
      socket.on('close', () => {
        ...
        if (this._activeConnections.size === 0 && !this._exiting && !this._busy) {
  ...
  async superUserHandler (req, res) {
    ...
    await fs.writeFile(path.resolve(confDir, '.superuser'), JSON.stringify({ email, password }, null, 2), { mode: 0o600 })
    res.json({ password })
  }
