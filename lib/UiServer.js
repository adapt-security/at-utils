...
class UiServer extends EventEmitter {
  constructor({ cwd, action, includePrereleases }) {
    super();
  ...
  async downloadHandler(req, res) {
    const opts = { includePrereleases: this.includePrereleases };
    const [latestRelease] = await Utils.getReleases(opts);
  ...
  async getReleasesHandler(req, res) {
    ...
    res.json({
      currentVersion,
      releases: await Utils.getReleases({ includePrereleases: this.includePrereleases, currentVersion })
    });
  }
