...
export default class UiServer extends EventEmitter {
  constructor (options) {
    ...
    if (this.options.action === 'install') { // install-specific endpoints
      ...
      this.get('/checkpoint', this.getCheckpointHandler.bind(this))
      this.post('/checkpoint', this.saveCheckpointHandler.bind(this))
    }
  ...
  async downloadHandler (req, res) {
    ...
    try {
      ...
      if (e.code !== 'GITCLONEEEXIST' || !this.options.resumeStep) throw e
      console.log('Repository already exists from previous install, reinstalling dependencies')
      await Utils.exec('npm ci', this.options.cwd)
    }
    res.end()
  }
  ...
  async getCheckpointHandler (req, res) {
    const state = await Utils.getInstallState(this.options.cwd)
    if (state) {
      res.json(state)
    } else {
      res.send(undefined, 404)
  ...
  async saveCheckpointHandler (req, res) {
    await Utils.saveInstallState(this.options.cwd, req.body)
    res.send()
  }
}
