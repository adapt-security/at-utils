...
class UiServer {
  ...
  async requestHandler(req, res) {
    ...
    if(handler) {
      ...
      return h?.catch?.(e => res.send(JSON.stringify(e), e.statusCode || 500));
    }
  ...
  registerUserHandler(req, res) {
    Utils.registerUser(req.body).then(() => res.send(undefined, 201));
  }
  ...
  async downloadHandler(req, res) {
    ...
    Utils.cloneRepo(latestRelease.name, this.rootDir).then(() => res.json(latestRelease));
  }
