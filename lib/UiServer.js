...
export default class UiServer extends EventEmitter {
  ...
  async registerUserHandler(req, res) {
    try {
      await Utils.registerSuperUser(this.options.cwd, req.body);
  ...
  async getReleasesHandler(req, res) {
    let currentVersion;
    try {
      const { version } = await Utils.loadPackage(this.options.cwd);
  ...
  async updateHandler(req, res) {
    await Utils.updateRepo(req.query.version, this.options.cwd);
    res.end();
  }
