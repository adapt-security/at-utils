...
export default class UiServer extends EventEmitter {
  ...
  async registerUserHandler (req, res) {
    try {
      await registerSuperUser({ ...this.options, superEmail: req.body.email, superPassword: req.body.password })
      res.send(undefined, 201)
    } catch (e) {
      const app = await startApp(this.options)
  ...
  async prereqHandler (req, res) {
    const checkPrerequisites = (await import('./utils/checkPrerequisites.js')).default
    await checkPrerequisites(this.options)
    res.send()
  }
  ...
  async getModulesHandler (req, res) {
    res.json((await getAppDependencies(this.options.cwd)).adapt)
  }

  async installModulesHandler (req, res) {
    await installLocalModules({ ...this.options, modules: req.body })
    res.send()
  }
